  ...
  renderHeaderView(username, avatar) {
    return (
      <View>
        ...
        <FlatList data={this.props.tableStore.momentList.slice()} renderItem={this.renderItem} />
      </View>
    )
  }
  ...
  renderReplys(item) {
    ...
    if (!Utils.isEmpty(arr) && arr.length > 0) {
      for (let i = 0; i < arr.length; i++) {
        replys.push(
          <Text key={item.item.moment_id + '-' + i} style={listItemStyle.commentText}>
            {arr[i].username + '：' + Base64Utils.decoder(arr[i].content)}
          </Text>
        )
      }
    }
                  ...
                  {Utils.isEmpty(item.item.favor_names) ? null : (
                    <View style={{ flexDirection: 'column' }}>
                      <View style={listItemStyle.favorContainer}>
                        <Image style={listItemStyle.favorImg} source={require('../../../img/ic_favor.png')} />
